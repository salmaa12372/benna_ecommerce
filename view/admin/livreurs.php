...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($act==='create') {
        ...
        header("Location: ".BASE."/view/admin/livreurs.php"); exit();
    }
    if ($act==='update') {
...
<div class="card">
  <div class="card-title">
    <i class="fas fa-truck"></i> Équipe de livraison
    <span class="card-title-count"><?= count($livreurs) ?> livreur(s)</span>
  </div>
    ...
    <table class="data-table">
      ...
      <tr class="usine-row">
        ...
        <td class="usine-actions">
          <button class="btn-usine-edit" onclick="openEdit(<?= htmlspecialchars(json_encode($u)) ?>,'livreur')">
          ...
          <form method="POST" class="form-inline">
            ...
            <input type="hidden" name="uid" value="<?= $u['id'] ?>"/>
            <button type="submit" class="btn-usine-delete" onclick="return confirm('Supprimer ce livreur ?')">
      ...
      <?php if (empty($livreurs)): ?>
        <tr class="empty-row">
          <td colspan="7"><i class="fas fa-inbox"></i> Aucun livreur enregistré.</td>
        </tr>
      <?php endif; ?>
      </tbody>
    </table>
